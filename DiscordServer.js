...
class DiscordServer {
  ...
  */
  ...
  async announce (title, text, options = {}) {
    const embed = {
      color: options.important ? 0x007552 : 0x0064ba,
      title,
      description: text
    }
    ...
    if (options.important) {
      ...
      if (this.server.systemChannel) {
        try {
          await this.server.systemChannel.send(`${this.server.owner} An important notice was triggered and there is no announcement channel configured and the server owner will not accept DMs from Trojan, so it has been posted here:`, { embed })
          return
        } catch (e) {}
      }
      ...
      if (options.lastResortChannel) {
        try {
          await options.lastResortChannel.send(`${this.server.owner} An important notice was triggered and there is no announcement channel configured and the server owner will not accept DMs from Trojan, so it has been posted here as a last resort:`, { embed })
        } catch (e) {}
      }
    }
  }
