...
class UpdateCommand extends Command {
  ...
  hasPermission (msg) {
    return this.client.isOwner(msg.author) || msg.member.hasPermission(this.userPermissions) || msg.member.roles.cache.find(role => role.name === 'Trojan Admin') || msg.member.roles.cache.find(role => role.name === 'Trojan Updater')
  }
  ...
  async fn (msg, args) {
    ...
    if (!(target instanceof Role)) { // They want to update a specific user (roles have .hoist, users do not)
      ...
      return msg.reply('Sorry, you can only update one user at a time.') //updating more than one user is only available with Trojan Plus: <https://www.patreon.com/erynlynn>.')
    }*/ else { // They want to update a whole role (premium feature)
    return msg.reply('Sorry, you can only update one user at a time.')
      ...
      if (affectedCount > limit) {
        return msg.reply(`Sorry, but Trojan only supports updating up to ${limit} members at once. Updating this role would affect approximately ${affectedCount} members.`)
      }
