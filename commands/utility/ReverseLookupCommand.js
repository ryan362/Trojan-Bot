...
class LookupCommand extends Command {
  ...
  hasPermission (msg) {
    return this.client.isOwner(msg.author) || msg.member.hasPermission(this.userPermissions) || msg.member.roles.cache.find(role => role.name === 'Trojan Admin') || msg.member.roles.cache.find(role => role.name === 'Trojan Reverse Lookup')
  }

  async fn (msg, args) {
    if (!config.reverseLookupApiKey) {
      return msg.reply('Sorry, reverse lookup is only available on official versions of Trojan.')
    }
