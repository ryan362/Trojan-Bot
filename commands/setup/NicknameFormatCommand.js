...
class NicknameFormatCommand extends Command {
  constructor (client) {
    super(client, {
      ...
      aliases: ['trojannicknameformat'],
      description: "`<format>` Set the format Trojan will use when setting users' nicknames. Available replacements are %USERNAME%, %RANK%, %USERID%, %SERVER%, %DISCORDNAME%, and %DISCORDID%. Example:` %USERNAME% - (%USERID%)`. Default `%USERNAME%`.",
			group: "setup",
      args: [
        {
          key: 'format',
