...
//    this.patronTransfers = {}
  ...
  initialize () {
    this.bot = new Discord.Client({
      ...
      disabledEvents: ['TYPING_START', 'VOICE_STATE_UPDATE', 'PRESENCE_UPDATE', 'MESSAGE_DELETE', 'MESSAGE_UPDATE', 'CHANNEL_PINS_UPDATE', 'MESSAGE_REACTION_REMOVE', 'MESSAGE_REACTION_REMOVE_ALL', 'CHANNEL_PINS_UPDATE', 'MESSAGE_DELETE_BULK', 'WEBHOOKS_UPDATE'],
      ...
      this.bot.dispatcher.addInhibitor(msg => {
        if (msg.guild && !this.authorizedOwners.includes(msg.guild.ownerID)) {
          if (this.authorizedOwners.length === 0) {
            ...
            msg.reply(`Sorry, this server isn't authorized.`) //to use Trojan Plus.${msg.member.hasPermission(['MANAGE_GUILD']) ? ' The server owner needs to donate at <https://www.patreon.com/erynlynn>, or you can invite the regular Trojan bot at <https://Trojan.link>.' : ''} If you are a patron encountering this issue, try running '${this.bot.commandPrefix}transferplus ${msg.guild.ownerID}'`) // notify sender to donate only if they're an "admin"
          }
...
*/
  ...
  ready () {
    ...
    this.bot.user.setActivity('https://glitch.trojan.technology/discord', { type: "LISTENING" })
  }
  ...
  async message (message) {
    ...
    if (server.getSetting('verifyChannel') === message.channel.id && message.cleanContent.toLowerCase() !== message.guild.commandPrefix + 'verify' && !(this.bot.isOwner(message.author) || message.member.hasPermission('MANAGE_GUILD') || message.member.roles.cache.find(role => role.name === 'Trojan Admin'))) {
  ...
  setActivity (text, activityType) {
    ...
    this.bot.user.setActivity(text || 'https://glitch.trojan.technology/discord', { type: activityType })
  }
