  ...
  async verify (options) {
    ...
    */
    ...
    if (this.user.bot) {
      return status({
        status: false,
        error: ':robot: Trojan cannot verify bots.'
      })
    }

    // Ignore users with this specific role (to give server owners more power)
    if (this.member.roles.cache.find(role => role.name === 'Trojan Bypass')) {
      return status({
        status: false,
        error: ':octagonal_sign: Trojan cannot act on users with the "Trojan Bypass" role.',
        nonFatal: true
      })
    }
    ...
    if (!this.member.manageable || !botMember.hasPermission('MANAGE_ROLES')) {
      return status({
        status: false,
        error: this.member.guild.ownerID === this.member.id ? '\n\nYou are the server owner. Trojan cannot make changes to you. This is a Discord restriction. If you want, you can change your own nickname.' : "\n\nTrojan can't manage this user. Please have a server admin drag Trojan's role above all other roles and ensure Trojan has permission to modify roles in order to fix this problem.",
        nonFatal: true
      })
    }
    ...
    if (data.status === 'ok') {
        ...
        if (!this.member.roles.cache.has(role) && this.server.roles.cache.has(role) && this.discordServer.canManageRole(role)) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: "There was an error while trying to assign the verified role! Ensure Trojan's role is above it." + errorAppend
            })
          }
        }
      }
        ...
        if (this.member.roles.cache.has(role) && this.server.roles.cache.has(role) && this.discordServer.canManageRole(role)) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: "There was an error while trying to remove the verified role! Ensure Trojan's role is above it." + errorAppend
            })
          }
        }
      }
      ...
      if (
        ...
        !this.member.roles.cache.find(role => role.name === 'Trojan Nickname Bypass') &&
        ...
        if (this.member.displayName !== nickname) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: this.member.guild.ownerID === this.member.id ? "Sorry, Discord doesn't allow bots to change the server owner's nickname. Please manually update your nickname. Or don't, I'm just an error message." : "Trojan doesn't have permission to change that user's nickname." + errorAppend
            })
          }
        }
      }
